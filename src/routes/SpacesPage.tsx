    ...
    <div>
        ...
        <section className="mb-5 rounded-lg border border-accent/30 bg-accent/10 p-4">
          ...
          <div className="space-y-2">
            {invitesForMyOrg.map(({ space, fromOrg }) => (
              <div key={space.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
                <div>
      ...
      <section className="mb-5 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        ...
        <div className="grid gap-3">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="space name" className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2.5 text-sm outline-none focus:border-accent/50" />
      ...
      <div className="space-y-2.5">
        {spaces.map((space) => {
          ...
          return (
            <Link key={space.id} to="/spaces/$slug" params={{ slug: space.slug }} className="group block rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4 transition hover:border-accent/40 hover:bg-[var(--color-surface-2)]">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <h3 className="text-[15px] font-semibold text-fg">{space.name}</h3>
                  {space.description && <p className="mt-1 line-clamp-2 text-sm text-[var(--color-muted)]">{space.description}</p>}
                </div>
                <div className="flex shrink-0 flex-wrap justify-end gap-1.5">
                  {members.map(({ org, status }) => (
                    <div key={org.id} className="flex items-center gap-1" title={`${org.name}: ${status}`}>
                      <OrgSquare org={org} />
                      {status !== "active" && <span className="text-[10px] text-[var(--color-muted)]">{status}</span>}
                    </div>
                  ))}
                </div>
              </div>
