...
export function SendAgentButton({
  ...
  return (
    <span className="relative inline-block">
      ...
      {open && (
        <span className="absolute left-0 top-full z-10 mt-1 w-36 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] p-1 shadow-xl">
          {Object.keys(AGENT_HANDLES).map((handle) => (
            <button
              key={handle}
              onClick={() => void send(handle)}
              className="block w-full rounded-sm px-2 py-1.5 text-left text-xs text-[var(--color-muted)] transition hover:bg-accent/10 hover:text-accent-soft"
            >
